...
export const action: ActionFunction = async ({ request }) => {
  ...
  // can render it.
  if (typeof answer !== 'string') return json('Come on, at least try!', { status: 400 })

  if (answer !== 'egg') return json(`Sorry, ${answer} is not right.`, { status: 400 })
...
export default function ActionsDemo() {
  ...
  useEffect(() => {
    if (actionMessage && answerRef.current) answerRef.current.select()
  }, [actionMessage])
  ...
  return (
    <div className="remix__page">
      <main>
        <h2>Actions!</h2>
        <p>
          This form submission will send a post request that we handle in our `action`
          export. Any route can export an action to handle data mutations.
        </p>
      ...
      <aside>
        ...
        <ul>
          ...
          <li>
            API:{' '}
            <a href="https://remix.run/api/conventions#action">Route Action Export</a>
          </li>
          <li>
            API:{' '}
