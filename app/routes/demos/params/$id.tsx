...
export function CatchBoundary() {
  ...
  const message = (() => {
    switch (caught.status) {
      case 401:
        return (
          <p>
            Looks like you tried to visit a page that you do not have access to. Maybe ask
            the webmaster ({caught.data.webmasterEmail}) for access.
          </p>
        )
      case 404:
        return <p>Looks like you tried to visit a page that does not exist.</p>
      default:
        return (
          <p>
  ...
  return (
    <>
      ...
      <p>{message}</p>
      <p>
        (Isn&apos;t it cool that the user gets to stay in context and try a different link
        in the parts of the UI that didn&apos;t blow up?)
      </p>
    </>
  )
}
